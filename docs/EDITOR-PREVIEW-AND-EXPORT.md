...
Active strokes now reuse the unchanged fitting prefix and a viewport-sized background. The worker sleeps between sample batches, and the standalone view records only changing ink/overlays during the gesture. See [STROKE-LATENCY.md](STROKE-LATENCY.md) for before/after measurements, exact-fit and image-equivalence checks, and remaining first-frame and long-stroke costs.
...
During interactive playback, only the focused visible editor viewport receives new frames. If it is hidden, closed or minimized, playback transfers to the other visible canvas. With neither canvas visible, drawing and compositing are deferred while the transport/audio clock continues. Focus changes take effect without restarting playback. Inactive canvases retain a fixed snapshot for expose requests; their scene renderer and camera/HUD updates remain suspended. The separate detached Camera Preview window is frozen during playback. Pause, Stop and natural completion restore updates to all editing views.

The transport shows `Render … fps / Target … fps` and the active viewport. A compact measured/target FPS badge also appears beside the timecode in the active camera preview or canvas. Measurement counts distinct timeline frames actually painted by the selected viewport over a 500 ms sample, excludes duplicate repaints and exports, resets on viewport handoff, and reports zero when frames stop arriving. It measures Qt painting cadence, not display scanout. Timer wakeups are scheduled against the next frame boundary to avoid accumulating rounded millisecond intervals at rates such as 24 or 30 fps.
...
- PaintCanvasSmoke: 71 offscreen checks, including both drawing origins, held-gesture deferral, rapid strokes, late completion after panel navigation, focus loss, thumbnail suspension, export overlay isolation, frame formatting, grouped light-table fading, reference-image fading, active-layer changes, shared toggle/history controls, Font Awesome icons, frame-boundary scheduling, measured FPS accounting and FPS export isolation. Cached 300-stroke offscreen workspace redraw measured approximately 3 ms/frame in this fixture; this is not a full interactive latency benchmark.
- PlaybackSmoke: 44 checks with native audio enabled, including silent transport, pause/resume/stop/loop/seek, Shot List controls, 200 px docks, audio attachment without scrubbing, sample consumption and cross-window tool/light-table synchronization, actual per-viewport paint counts, focus/visibility/minimize/close fallback, hidden-view compositing deferral, frozen expose snapshots, and pause/stop restoration of both editors and the detached preview. The synthetic tone produced a meter reading around -28 dB. In the native animated-panel fixture, the workspace painted 18 frames during a 700 ms sample while the integrated canvas received zero scheduled paints; measured rate was approximately 24.6 fps against a 25 fps target. The live native workspace FPS badge was also inspected at 25.0 / 25 fps. These measurements describe this fixture, not a production-project benchmark.
...
The new ShotListPresentation and MoviePlayerWindow sources, PlaybackTiming header, and Qt Multimedia modules are included in the active Qt 6 project and root project definition. Existing user project files and unrelated untracked documents/build directories were preserved. B-Line settings constructors now honor the explicitly selected QSettings format: normal runs retain native settings, while smoke tests use temporary INI settings instead of changing the user's preferences.

For a 30-second isolated native playback review, run `./PlaybackSmoke --review`. It uses a temporary animated project and settings, then exits. `QWidget::grab()` of a parent can omit disabled child canvases during playback; inspect the native window or grab the canvas directly when checking its frozen contents.
