...
ShotListPresentation::ShotListPresentation(QTreeWidget *tree) : QObject(tree),m_tree(tree),m_toolbar(new QWidget(tree->parentWidget())) {
    ...
    QSettings settings("B-Line","Storyboard");m_mode=qBound(0,settings.value("shotList/view",1).toInt(),2);
    const QStringList modes{tr("Compact"),tr("Thumbnails"),tr("Detailed")};
    for(int mode=0;mode<modes.size();++mode) {auto *a=menu->addAction(modes[mode]);a->setCheckable(true);a->setChecked(mode==m_mode);group->addAction(a);connect(a,&QAction::triggered,this,[this,mode]{m_mode=mode;QSettings("B-Line","Storyboard").setValue("shotList/view",mode);applyView();});}
    menu->addSeparator();
    ...
    for(auto *a:{m_panels,m_dialogue}) connect(a,&QAction::toggled,this,[this]{QSettings s("B-Line","Storyboard");s.setValue("shotList/panels",m_panels->isChecked());s.setValue("shotList/dialogue",m_dialogue->isChecked());refresh();});
